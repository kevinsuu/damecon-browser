...
### v1.0.18 更新内容（v1.0.17 以降）

- 艦娘図鑑の改造資材ツールチップで、各資材のアイコン、ローカライズ名、必要数を表示します。KC3 の高解像度特殊資材アイコンにも対応します。
- BrowserView の KC3 タブから拡張機能タブを開く操作が失敗する問題を修正しました。

### v1.0.17 更新内容（v1.0.16 以降）

- 遠征推薦の最適プランに、燃料・弾薬・鋼材・ボーキサイト・バケツの合計毎時純収益を表示します。
  各カードの回収間隔と推定補給コストを使い、艦隊が遠征中の場合は現在の帰還待ち時間を明示的に除外します。
...
10. **[艦娘図鑑の改造資材](./docs/master-ship-remodel-materials.md)** — 艦娘図鑑の改造ツールチップに、特殊資材ごとのアイコン、ローカライズ名、必要数を表示します。
...
- [x] 艦娘図鑑の改造要件で、特殊資材ごとのアイコン、ローカライズ名、必要数を表示
